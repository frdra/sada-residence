...
export default function AttendancePage() {
  ...
  const statusLabels: Record<string, { label: string; color: string }> = {
    ...
    leave: { label: "Cuti", color: "bg-blue-100 text-terracotta-700" },
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="text-center py-4">
        ...
        <p className="text-4xl font-bold text-terracotta-500 mt-1 font-mono">{currentTime}</p>
        {settings && (
      ...
      {/* Tabs */}
      <div className="flex gap-2 bg-concrete-100 rounded-xl p-1">
        <button
          onClick={() => setTab("absen")}
          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${tab === "absen" ? "bg-white text-terracotta-500 shadow-sm" : "text-concrete-600"}`}
        >
          Absensi
        </button>
        <button
          onClick={() => setTab("riwayat")}
          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${tab === "riwayat" ? "bg-white text-terracotta-500 shadow-sm" : "text-concrete-600"}`}
        >
          Riwayat
        </button>
      </div>
      ...
      {tab === "absen" && !showCamera && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border">
            <h3 className="font-semibold text-charcoal-800 mb-4">Status Hari Ini</h3>
            <div className="grid grid-cols-2 gap-4">
              {/* Clock In */}
              <div className={`rounded-xl p-4 text-center ${today?.clock_in ? "bg-green-50" : "bg-concrete-100"}`}>
                <p className="text-xs text-concrete-600 mb-1">Masuk</p>
              ...
              {/* Clock Out */}
              <div className={`rounded-xl p-4 text-center ${today?.clock_out ? "bg-terracotta-50" : "bg-concrete-100"}`}>
                <p className="text-xs text-concrete-600 mb-1">Pulang</p>
                <p className={`text-xl font-bold ${today?.clock_out ? "text-terracotta-500" : "text-gray-300"}`}>
                  {formatTime(today?.clock_out ?? null)}
                </p>
                {today?.work_duration_minutes && (
                  <p className="text-[10px] text-concrete-600 font-medium mt-1">
            ...
            {today?.clock_in_address && (
              <p className="text-[10px] text-concrete-600 mt-3 truncate">
          ...
          {/* Property selection */}
          {properties.length > 1 && (
            <div className="bg-white rounded-xl p-4 shadow-sm border border-concrete-200">
              <label className="block text-sm font-medium text-charcoal-800 mb-2">Lokasi Properti</label>
              <select
                className="w-full border border-concrete-200 rounded-xl px-4 py-2.5 text-sm"
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              ...
              className={`p-6 rounded-2xl text-center font-semibold shadow-sm transition-all ${
                today?.clock_in
                  ? "bg-concrete-100 text-concrete-600 cursor-not-allowed"
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-concrete-600 hover:text-charcoal-600 text-xl"
            >
              &times;
            </button>
          </div>
              ...
              <>
                ...
                <div className="absolute bottom-4 left-0 right-0 flex justify-center">
                  <button
                    ...
                    className="w-16 h-16 rounded-full bg-white border-4 border-concrete-300 shadow-lg active:scale-90 transition-transform"
                  >
          ...
          {capturedPhoto && (
            <button
              ...
              className="w-full py-4 rounded-2xl font-semibold text-white text-base transition-all disabled:opacity-50 bg-terracotta-500 hover:bg-terracotta-600 active:scale-[0.98]"
            >
              {submitting
                ? "Mengirim..."
                : !gpsLocation
      ...
      {tab === "riwayat" && (
        <div className="space-y-3">
          {history.length === 0 ? (
            <div className="text-center py-12 text-concrete-600">
              ...
              return (
                <div key={a.id} className="bg-white rounded-xl p-4 shadow-sm border">
                  ...
                  <div className="grid grid-cols-3 gap-3 text-center text-xs">
                    <div>
                      <p className="text-concrete-600">Masuk</p>
                      <p className="font-semibold text-green-600">{formatTime(a.clock_in)}</p>
                    </div>
                    <div>
                      <p className="text-concrete-600">Pulang</p>
                      <p className="font-semibold text-terracotta-500">{formatTime(a.clock_out)}</p>
                    </div>
                    <div>
                      <p className="text-concrete-600">Durasi</p>
