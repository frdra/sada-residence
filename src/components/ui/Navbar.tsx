      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="relative w-10 h-10">
              <svg viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
                <circle cx="20" cy="20" r="18.5" stroke={scrolled ? "#A0695A" : "#C4907F"} strokeWidth="1" />
                <rect x="14" y="10" width="12" height="16" rx="0.5" fill="none" stroke={scrolled ? "#2C2C2C" : "#E8E4DE"} strokeWidth="1" />
                <rect x="17.5" y="7.5" width="5" height="3.5" rx="0.5" fill={scrolled ? "#A0695A" : "#C4907F"} />
                <rect x="10" y="16" width="20" height="0.8" rx="0.4" fill={scrolled ? "#A0695A" : "#C4907F"} opacity="0.7" />
                <rect x="10" y="21" width="20" height="0.8" rx="0.4" fill={scrolled ? "#A0695A" : "#C4907F"} opacity="0.7" />
                <path d="M8,30 C13,27 18,32 23,29 C27,27 30,30 33,29" stroke={scrolled ? "#7A8B6F" : "#A3B296"} strokeWidth="1" fill="none" strokeLinecap="round" />
              </svg>
            </div>
            <div>
              <span className={`font-display text-xl font-light tracking-[0.3em] transition-colors duration-500 ${scrolled ? "text-charcoal-800" : "text-concrete-200"}`}>
                SADA
              </span>
              <span className={`block font-body text-[9px] font-normal tracking-[0.35em] transition-colors duration-500 ${scrolled ? "text-terracotta-500" : "text-terracotta-400"}`}>
                RESIDENCE
              </span>
            </div>
          </Link>
            ...
            {[
              ...
              <Link
                ...
                className={`text-sm font-body font-medium transition-colors duration-300 relative group ${
                  scrolled ? "text-charcoal-600 hover:text-charcoal-800" : "text-white/70 hover:text-white"
                }`}
              >
                {link.label}
          ...
          <button
            ...
            className={`md:hidden p-2.5 rounded-xl transition-colors ${scrolled ? "text-charcoal-600" : "text-white/80"}`}
